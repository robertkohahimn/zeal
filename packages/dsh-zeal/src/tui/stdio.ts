 ...
 * `process.stderr.write` and `console.log`/`console.warn`/`console.error` so
 * every diagnostic instead lands in one append-only log file under the
 * profile directory, and hands back a restore function that undoes exactly
 * that patch (used on quit, before the terminal is handed back).
 *
...
/** `console.log`/`warn`/`error`'s shared signature. */
type ConsoleMethod = (...args: unknown[]) => void

/**
 * Redirect `process.stderr.write` and `console.log`/`warn`/`error` so their
 * output is appended to `logPath` instead of reaching the terminal. Creates
 * `logPath`'s parent directory if it does not already exist.
...
export function redirectDiagnostics(logPath: string): () => void {
  ...
  console.log = (...args: unknown[]): void => appendLine(format(...args))
  console.warn = (...args: unknown[]): void => appendLine(format(...args))
  console.error = (...args: unknown[]): void => appendLine(format(...args))
